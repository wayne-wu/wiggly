...
# Construct mass and stiffness matrices (for 2 & 3 spring cases)
def get_mass_stiffness_mats(m_vals, k_vals):
    ...
    if len(k_vals) == 1:
        ...
        k1, k2 = k_vals[0], k_vals[1]
        K = np.array([[-k1, k1, 0], [k1, -k1-k2, k2], [0, k2, -k2]])

    return np.diag(m_vals), K
...
# Set keyframes
key1 = (0, [0, 0, 0], [0, 0, 0])
partial_key2 = (0.5, [None, None, 5], [None, None, None])   # Try: t=0.5: [5, None, None] & [None, None, 5], t=0.2: [None, None, -5], & t=0.5: [None, -2, 5] doesn't properly do it unless change energy
key3 = (1, [1, -3, 2], [0, 0, 0])
...
def run_step():
    ...
    global first_run, keyframes, coeffs, eigvecs, eigvals, lambs, deltas, scale_x, scale_y,\
           pt_radius, tg_linelen, sample_size, t_start, t_end, m, d, x0, cube_apothem, dt, t_curr, sim_y, sim_scale
    ...
    # Compute cube positions + plot
    pos = x0 + u(t_curr, deltas, lambs, g, eigvals, eigvecs, keyframes, coeffs)
    for i in range(d-1):
        x1, x2 = pos[i] * sim_scale, pos[i+1] * sim_scale
        if first_run:
            w.create_line(*A(x1, sim_y), *A(x2, sim_y), fill='white', tags='spring'+str(i))
        else:
            w.coords(w.find_withtag('spring'+str(i)), *A(x1, sim_y), *A(x2, sim_y))

    # Plot springs
    for i, val in enumerate(pos):
        x = val * sim_scale  # scaling factor
        if first_run:
            w.create_rectangle(*A(x-cube_apothem, sim_y-cube_apothem), *A(x+cube_apothem, sim_y+cube_apothem), fill=colors[i], outline='white', tags='cube'+str(i))
        else:
            w.coords(w.find_withtag('cube'+str(i)), *A(x-cube_apothem, sim_y-cube_apothem), *A(x+cube_apothem, sim_y+cube_apothem))

    # Plot keyframe bars
    for i, frame in enumerate(keyframes):
        for j, disp in enumerate(frame[1]):
            if disp is not None:
                col = colors[j]
                if i == 0:
                    col = 'white'
                ...
                x = (x0[j] + disp) * sim_scale  # scale factor
                heightBy2 = cube_apothem * 2
                if first_run:
                    w.create_line(*A(x, sim_y+heightBy2), *A(x, sim_y-heightBy2), fill=col, tags='bar'+str(i)+str(j), width=2)
